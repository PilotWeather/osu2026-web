...
export default async function FlightsPage({ searchParams }: FlightsPageProps) {
  ...
  const flights = await prisma.flight.findMany({
    where: {
      ...
      ...(q.search ? { OR: [{ aircraft: { registration: { contains: q.search, mode: "insensitive" } } }, { studentName: { contains: q.search, mode: "insensitive" } }, { instructor: { OR: [{ firstName: { contains: q.search, mode: "insensitive" } }, { lastName: { contains: q.search, mode: "insensitive" } }] } }] } : {}),
    },
  ...
  const items: FlightListItem[] = flights.map(f=>({id:f.id,date:f.flightDate.toISOString().slice(0,10),sortie:f.sourceSortieNo??"-",aircraft:f.aircraft.registration,aircraftType:f.aircraft.aircraftType,instructor:f.instructor?`${f.instructor.firstName} ${f.instructor.lastName}`:"-",student:f.student?.displayName??f.studentName??"-",takeoff:formatOperationalTime(f.takeoffTime),landing:formatOperationalTime(f.landingTime),duration:formatDuration(f.sortieDurationMinutes),departure:f.departureAirport??"-",arrival:f.arrivalAirport??"-",rules:f.flightRules??"-",runway:f.runway??"-",frequency:f.frequency??"-",remarks:f.remarks??"-",airborne:formatDuration(f.airborneDurationMinutes),ground:formatDuration(f.groundDurationMinutes),batch:f.importBatch.originalFilename}));
