...
export default async function ImportReviewPage({ params, searchParams }: ImportReviewPageProps) {
  ...
  const [batch, personnel] = await Promise.all([
    ...
    prisma.personnel.findMany({ orderBy: [{ firstName: "asc" }, { lastName: "asc" }], select: { id: true, firstName: true, lastName: true, canonicalFullName: true } }),
  ]);
  if (!batch) notFound();
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 dark:bg-[#0b0f19] sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <form action={action} className="space-y-4"><section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-800/80"><tr><th className="px-4 py-3">Sorti</th><th className="px-4 py-3">Uçak</th><th className="px-4 py-3">Öğretmen</th><th className="px-4 py-3">Öğrenci</th><th className="px-4 py-3">Kalkış</th><th className="px-4 py-3">İniş</th><th className="px-4 py-3">Süre</th><th className="px-4 py-3">Durum</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{parsedRows.map(({row,data})=><tr key={row.id}><td className="px-4 py-3">{data.sourceSortieNo ?? "-"}</td><td className="px-4 py-3 font-medium">{data.aircraftRegistration ?? "-"}</td><td className="px-4 py-3"><div className="mb-1 text-xs text-slate-500">{data.instructorName ?? "Kaynakta yok"}</div><select name={`instructor_${row.id}`} defaultValue={row.instructorId ?? ""} disabled={data.flightStatus === "CANCELLED" || row.status === "INVALID" || batch.status === "IMPORTED"} className="max-w-52 rounded-lg border border-slate-300 bg-white px-2 py-1.5 dark:border-slate-700 dark:bg-slate-800"><option value="">Eşleşme seçin</option>{personnel.map(person=><option key={person.id} value={person.id}>{personnelDisplayName(person)}</option>)}</select></td><td className="px-4 py-3">{data.studentName ?? "-"}</td><td className="px-4 py-3">{data.takeoffTime ?? "-"}</td><td className="px-4 py-3">{data.landingTime ?? "-"}</td><td className="px-4 py-3">{formatDuration(data.sortieDurationMinutes)}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${row.status === "READY" || row.status === "IMPORTED" ? "bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300" : row.status === "CANCELLED" ? "bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300" : row.status === "INVALID" ? "bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300" : "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"}`}>{row.status}</span>{row.warning ? <p className="mt-1 max-w-52 text-xs text-amber-700 dark:text-amber-300">{row.warning}</p> : null}</td></tr>)}</tbody></table></div></section>
