...
export default async function ImportReviewPage({ params, searchParams }: ImportReviewPageProps) {
  ...
  const totalMinutes = parsedRows.reduce((sum, item) => sum + (item.data.sortieDurationMinutes ?? 0), 0);
  const ready = batch.rows.filter((row) => row.status === "READY").length;
  const warnings = batch.rows.filter((row) => row.status === "REVIEW").length;
  const invalid = batch.rows.filter((row) => row.status === "INVALID").length;
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 dark:bg-[#0b0f19] sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{[["Uçuş",batch.totalRows],["Hazır",ready],["Kontrol",warnings],["Geçersiz",invalid],["Toplam Sorti",formatDuration(totalMinutes)]].map(([label,value])=><div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>)}</div>
    <form action={action} className="space-y-4"><section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-800/80"><tr><th className="px-4 py-3">Sorti</th><th className="px-4 py-3">Uçak</th><th className="px-4 py-3">Öğretmen</th><th className="px-4 py-3">Öğrenci</th><th className="px-4 py-3">Kalkış</th><th className="px-4 py-3">İniş</th><th className="px-4 py-3">Süre</th><th className="px-4 py-3">Durum</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{parsedRows.map(({row,data})=><tr key={row.id}><td className="px-4 py-3">{data.sourceSortieNo ?? "-"}</td><td className="px-4 py-3 font-medium">{data.aircraftRegistration ?? "-"}</td><td className="px-4 py-3"><div className="mb-1 text-xs text-slate-500">{data.instructorName ?? "Kaynakta yok"}</div><select name={`instructor_${row.id}`} defaultValue={row.instructorId ?? ""} disabled={row.status === "INVALID" || batch.status === "IMPORTED"} className="max-w-52 rounded-lg border border-slate-300 bg-white px-2 py-1.5 dark:border-slate-700 dark:bg-slate-800"><option value="">Eşleşme seçin</option>{personnel.map(person=><option key={person.id} value={person.id}>{person.firstName} {person.lastName}</option>)}</select></td><td className="px-4 py-3">{data.studentName ?? "-"}</td><td className="px-4 py-3">{data.takeoffTime ?? "-"}</td><td className="px-4 py-3">{data.landingTime ?? "-"}</td><td className="px-4 py-3">{formatDuration(data.sortieDurationMinutes)}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${row.status === "READY" || row.status === "IMPORTED" ? "bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300" : row.status === "INVALID" ? "bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-300" : "bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"}`}>{row.status}</span>{row.warning ? <p className="mt-1 max-w-52 text-xs text-amber-700 dark:text-amber-300">{row.warning}</p> : null}</td></tr>)}</tbody></table></div></section>
      {batch.status !== "IMPORTED" && can(user.role,"IMPORT_PDF") ? <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><p className="text-sm text-slate-600 dark:text-slate-400">{batch.validRows} kayıt · {formatDuration(totalMinutes)} · {batch.warningRows} uyarı</p><button type="submit" className="rounded-xl bg-blue-600 px-5 py-2.5 font-semibold text-white hover:bg-blue-700">Uçuşları İçe Aktar</button></div> : null}
    </form>
  </div></main>;
}
